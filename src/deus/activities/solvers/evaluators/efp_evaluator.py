...
from deus.activities.solvers.evaluators.efp_data_handler import\
    EFPEvalDataHandler
from deus.activities.solvers.evaluators.efp_script_handler import\
...
class EFPEvaluator(ExpensiveFunctionEvaluator):
    ...
    def set_eval_options(self, eval_options):
        ...
        if self._eval_method == "serial":
            mkeys = ['ufunc_ptr', 'store_constraints', 'acceleration']
            assert all(mkey in eval_options.keys() for mkey in mkeys),\
                "eval options keys must be:\n" \
                "'ufunc_ptr', 'store_constraints', 'acceleration'"

        elif self._eval_method == "mppool":
            # assert False, "Not implemented yet"
            mkeys = ['pool_size', 'store_constraints', 'acceleration']
            assert all(mkey in eval_options.keys() for mkey in mkeys)
    ...
    def set_worst_efp(self, worst):
        self._data_handler.set_worst_efp(worst)
    ...
    def _expensive_func(self, inputs):
        ...
        worst_efp = self._data_handler.get_worst_efp()
